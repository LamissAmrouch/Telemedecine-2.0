...
{ 
	...
	$("#allergie").change(function(event) {
		/* Act on the event */
...
$option = '<option class=\'anticident\' hidden>';
/*
  <?php  
  $option = '';      
 $sql2 = "SELECT nom_ant FROM anticident"; 
 $result2 = mysql_query($sql2); 
 while ( $row2 = mysql_fetch_array($result2)) 
 { 
     $option = $option+$row2[0];
echo  $option;
 } 
...
$option =$option+ '</option>';
		$html = '<div><label for="anticident" class="col-sm-4 control-label"></label>'+
		'<div class="col-sm-8" ><div class=" input-group" ><select class="chosen-select form-control" id="anticident" '+
		'name="anticident[]" placeholder="Type Anticident">'+$option+'</select> <span class="input-group-btn"><button class="delete btn btn-danger">'+
		'<i class="fa fa-minus-circle fa-fw"></i></button></span></div></div></div>';
